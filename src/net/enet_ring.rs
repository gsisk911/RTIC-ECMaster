//! ENET DMA buffer descriptor rings.
//!
//! Vendored from `rt1062-eth-rs` by Tim Vrakas (MIT license).
//! Adapted: removed `board` dependency, removed `print_dt` debug helper.

use core::ptr::null;
use static_assertions::assert_eq_size;
use struct_pad::*;

/// RX buffer descriptor (enhanced format, 32 bytes).
#[repr(C)]
#[derive(Clone, Copy)]
#[repr(align(8))]
...
/// TX buffer descriptor (enhanced format, 32 bytes).
#[repr(C)]
#[derive(Clone, Copy)]
#[repr(align(8))]
...
/// RX descriptor table: descriptors + buffers, 64-byte aligned for DMA.
#[repr(C)]
#[repr(align(64))]
...
/// TX descriptor table: descriptors + buffers, 64-byte aligned for DMA.
#[repr(C)]
#[repr(align(64))]
...
impl<const MTU: usize, const LEN: usize> RxDT<MTU, LEN> {
    pub const fn default() -> RxDT<MTU, LEN> {
        RxDT {
            desc: [RxDescriptor {
                len: 0,
                flags: 0x8000, // E (empty) bit set — owned by ENET
                buffer: null(),
                flags2: 0x0000,
                flags3: 0x0000,
                checksum: 0x0000,
